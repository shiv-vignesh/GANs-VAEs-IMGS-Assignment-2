...
def init_optimizer_mnist(model:GAN_MNIST, generator_lr:float, discrim_lr:float):
...
def train_discriminator(model:GAN_MNIST, 
    ...
    outputs = model.forward_discriminator(original_images)
    d_loss_real = criterion(outputs, real_labels)

    # Train on fake images
    latent_x = torch.randn(batch_size, model.generator.latent_size).to(device)
...
def train_generator(model:GAN_MNIST, 
    ...
    # Train generator to maximize log(D(G(z))) instead of log(1 - D(G(z)))
...
def train_mnist(model:GAN_MNIST, 
    ...
    for cur_epoch in range(num_epochs):   
        ...
        if (cur_epoch + 1) % 50 == 0:
            ...
            z = torch.randn(batch_size, model.generator.latent_size).to(device)
            generated_images = model.generator(z).view(-1, 1, 28, 28)
